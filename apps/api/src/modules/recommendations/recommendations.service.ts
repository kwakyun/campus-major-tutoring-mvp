...
import { CandidateQueryService } from "../catalog/candidate-query.service";
import { TutorAvailabilityRepository } from "../catalog/tutor-availability.repository";
import {
  RecommendationQuery,
...
export class RecommendationsService {
  ...
  private readonly eventQueue: Array<RecommendationEventInput & { receivedAt: string }> = [];

  constructor(
    private readonly candidateQuery: CandidateQueryService,
    private readonly availabilityRepo: TutorAvailabilityRepository,
  ) {}

  findRecommendations(query: RecommendationQuery): RecommendationResponse {
    ...
    const evaluated: RecommendedCandidate[] = [];
    ...
    for (const { course, timeMatches } of rawCandidates) {
      ...
      // 입문/초급 수준 매칭 사유
      const isUserIntro =
        query.level?.toLowerCase() === "introductory" ||
    ...
    let alternativeTimeSlots: AlternativeTimeSlot[] | undefined;
    let waitlistPath: string | undefined;
    ...
    if (evaluated.length === 0) {
      ...
      if (query.subjectId) qParams.set("subjectId", query.subjectId);
      if (query.lifeZoneId) qParams.set("lifeZoneId", query.lifeZoneId);
  ...
  private findAlternativeSlots(lifeZoneId: string, subjectId?: string): AlternativeTimeSlot[] {
  ...
  recordEvent(event: RecommendationEventInput): void {
    ...
    this.logger.debug(
      `[EventTracked] ${event.eventType} for session ${event.recommendationSessionId} (queue: ${this.eventQueue.length})`,
    );
  }
