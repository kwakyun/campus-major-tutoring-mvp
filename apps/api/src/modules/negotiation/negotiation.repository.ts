...
export class NegotiationRepository {
  ...
  private messagesByConversation = new Map<string, ChatMessageRecord[]>();
  ...
  createConversation(
    ...
    if (tutorId === learnerId) {
      throw new DomainError(
        ...
        "자기 자신의 개설 수업과는 협의하거나 거래할 수 없습니다.",
      );
    }
    ...
    this.addMessage(
      ...
      "system",
      "system",
      "협의방이 생성되었습니다. 수업 일정, 장소, 학습 범위 등을 협의하고 최종 제안서에 합의해주세요.",
    );
  ...
  listConversationsByUser(userId: string): ConversationRecord[] {
    return Array.from(this.conversations.values()).filter(
      (c) => c.learnerId === userId || c.tutorId === userId,
    );
  }
  ...
  addMessage(
    ...
    if (typeof text !== "string" || !text.trim() || text.length > 5000) {
      throw new DomainError("VALIDATION_ERROR", "메시지는 1~5000자로 입력해주세요.");
    }
  ...
  createProposal(
    ...
    const conv = this.conversations.get(conversationId);
    if (!conv) {
      throw new DomainError("NOT_FOUND", "해당 협의방을 찾을 수 없습니다.");
    }
    if ((isTutor ? conv.tutorId : conv.learnerId) !== proposedBy) {
      throw new DomainError("FORBIDDEN", "협의 당사자만 제안할 수 있습니다.", 403);
    }
    if (conv.status !== "active") {
      throw new DomainError("CONDITION_NOT_MET", "종료되거나 이미 예약된 협의방에서는 제안서를 발행할 수 없습니다.");
    }
    ...
    if (!Number.isFinite(start) || !Number.isFinite(end) || minutes <= 0 || !Number.isInteger(minutes)) {
      throw new DomainError("VALIDATION_ERROR", "수업 시작·종료 시간을 올바른 분 단위로 입력해주세요.");
    }
    if (input.totalMinutes !== undefined && input.totalMinutes !== minutes) {
      throw new DomainError("VALIDATION_ERROR", "수업 시간이 시작·종료 시간과 일치하지 않습니다.");
    }
    if (!Number.isSafeInteger(input.agreedPriceAmount) || input.agreedPriceAmount < 0) {
      throw new DomainError("VALIDATION_ERROR", "금액은 0 이상의 정수여야 합니다.");
    }
    if (typeof input.location !== "string" || !input.location.trim()) {
      throw new DomainError("VALIDATION_ERROR", "수업 장소를 입력해주세요.");
    }
    ...
    this.addMessage(
      ...
      `제안서(버전 ${version})가 발행되었습니다: ₩${input.agreedPriceAmount.toLocaleString()}원 / ${input.location}`,
      proposalId,
    );
  ...
  agreeProposal(proposalId: string, userId: string, isTutor: boolean): ProposalRecord {
    const proposal = this.proposals.get(proposalId);
    if (!proposal) {
      throw new DomainError("NOT_FOUND", "해당 제안서를 찾을 수 없습니다.");
    }
    ...
    if ((isTutor ? conv.tutorId : conv.learnerId) !== userId) {
      throw new DomainError("FORBIDDEN", "협의 당사자만 동의할 수 있습니다.", 403);
    }

    // 최신 제안서인지 검증 (구버전 수락 차단: STALE_PROPOSAL)
    if (conv.currentProposalId !== proposalId || proposal.status === "superseded") {
      throw new DomainError("STALE_PROPOSAL", "이전 버전의 제안서에는 동의할 수 없습니다. 최신 제안서를 확인하세요.");
    }
    ...
    if (proposal.agreedByLearner && proposal.agreedByTutor) {
      ...
      this.addMessage(
        ...
        "system",
        "system",
        `🎉 양측이 제안서(버전 ${proposal.version})에 모두 동의하여 최종 합의되었습니다! 이제 예약을 확정할 수 있습니다.`,
        proposal.id,
      );
    } else {
      this.addMessage(
        ...
        userId,
        isTutor ? "tutor" : "learner",
        `제안서(버전 ${proposal.version})에 동의했습니다. 상대방의 동의를 기다립니다.`,
        proposal.id,
      );
    }
