...
import { UnitBreakdownItem } from "./course.types";

export interface CurriculumDraftRequest {
  topic: string;
  subjectId?: string;
  level?: string;
  sessionCount?: number;
 ...
 */
@Injectable()
export class CurriculumDraftService {
  private readonly logger = new Logger(CurriculumDraftService.name);

  /** 실제 LLM 연동 전 임시 규칙 기반 생성기 버전 태그 — 응답에 그대로 노출해 투명성을 지킨다. */
  static readonly GENERATOR_VERSION = "rule-based-template-v1";

  private static readonly SESSION_TEMPLATE: Array<{ title: string; description: string }> = [
    {
      title: "핵심 개념 정리",
  ...
  generateDraft(input: CurriculumDraftRequest): CurriculumDraftResult {
    ...
    this.logger.debug(
      `[CurriculumDraft] topic="${topic}" level=${levelLabel} sessionCount=${sessionCount} generator=${CurriculumDraftService.GENERATOR_VERSION}`,
    );
    ...
    return {
      ...
      totalMinutes,
      curriculumSource: "ai_generated",
      generatorVersion: CurriculumDraftService.GENERATOR_VERSION,
      topic,
    };
  }

  private normalizeSessionCount(count?: number): number {
    if (!count || Number.isNaN(count)) return 4;
    return Math.min(Math.max(Math.trunc(count), 2), CurriculumDraftService.SESSION_TEMPLATE.length);
  }
  ...
  private buildUnitBreakdown(topic: string, sessionCount: number): UnitBreakdownItem[] {
    return CurriculumDraftService.SESSION_TEMPLATE.slice(0, sessionCount).map((item, idx) => ({
