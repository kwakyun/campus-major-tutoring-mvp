 ...
 */
const SEED_WINDOWS: Record<string, TimeWindow[]> = {
  "seed-tutor-0001": [
...
export class TutorAvailabilityRepository {
  private windowsByTutor = new Map<string, TimeWindow[]>(
    Object.entries(SEED_WINDOWS).map(([tutorId, windows]): [string, TimeWindow[]] => [
      tutorId,
      windows.map((w) => ({ ...w })),
    ]),
  );
  ...
  setWindows(tutorId: string, windows: TimeWindow[]): TimeWindow[] {
    for (const w of windows) {
      if (new Date(w.end).getTime() <= new Date(w.start).getTime()) {
        throw new DomainError("VALIDATION_ERROR", `잘못된 시간 구간입니다: ${w.start} ~ ${w.end}`);
      }
    }
  ...
  getWindows(tutorId: string): TimeWindow[] {
    return this.windowsByTutor.get(tutorId) ?? [];
  }

  /** 추천/매칭 모듈이 사용할 후보 조회 계약의 일부 — 특정 시간대와 겹치는 가능 시간이 있는지. */
  hasAvailabilityOverlapping(tutorId: string, window: TimeWindow): boolean {
