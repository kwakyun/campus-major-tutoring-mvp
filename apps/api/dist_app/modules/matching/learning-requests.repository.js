...
exports.LearningRequestsRepository = void 0;
...
let LearningRequestsRepository = class LearningRequestsRepository {
    constructor() {
        this.records = new Map(SEED_REQUESTS.map((r) => [r.id, r]));
        this.seq = 1000;
    }
    ...
    requireOwnedOrOperator(id, user) {
        const record = this.records.get(id);
        if (!record)
            throw new errors_1.DomainError("NOT_FOUND", "학습 요청을 찾을 수 없습니다.", 404);
        ...
        if (!isOwner && !isOperator) {
            throw new errors_1.DomainError("FORBIDDEN", "본인 또는 매칭 담당 운영자만 조회할 수 있습니다.", 403);
        }
        return record;
    }
    ...
    withdraw(id, learnerId) {
        ...
        if (record.learnerId !== learnerId) {
            throw new errors_1.DomainError("FORBIDDEN", "본인 학습 요청만 철회할 수 있습니다.", 403);
        }
     ...
     * 만들어내지 않고 상태·사유만 남긴다). */
    assist(id, nextStatus, reason, note) {
        const record = this.records.get(id);
        if (!record)
            throw new errors_1.DomainError("NOT_FOUND", "학습 요청을 찾을 수 없습니다.", 404);
        if (nextStatus === "waitlisted" || nextStatus === "expired") {
            if (!reason) {
                throw new errors_1.DomainError("VALIDATION_ERROR", "waitlisted/expired 전이에는 matchingFailureReason이 필요합니다.");
            }
        }
...
exports.LearningRequestsRepository = LearningRequestsRepository;
exports.LearningRequestsRepository = LearningRequestsRepository = __decorate([
