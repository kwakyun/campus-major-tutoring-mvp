...
let CoursesRepository = class CoursesRepository {
    ...
    listPublic(filter) {
        ...
        if (filter.subjectId)
            items = items.filter((c) => c.subjectId === filter.subjectId);
        if (filter.lifeZoneId)
            items = items.filter((c) => c.lifeZoneId === filter.lifeZoneId);
    ...
    update(id, tutorId, patch) {
        const record = this.requireOwned(id, tutorId);
        if (patch.status && patch.status !== record.status) {
            const allowed = course_types_1.COURSE_STATUS_TRANSITIONS[record.status];
            if (!allowed.includes(patch.status)) {
                throw new errors_1.DomainError("CONDITION_NOT_MET", `${record.status} 상태에서 ${patch.status}(으)로 전이할 수 없습니다.`, 409);
            }
            record.status = patch.status;
        }
        if (patch.subjectId !== undefined)
            record.subjectId = patch.subjectId;
        if (patch.lifeZoneId !== undefined)
            record.lifeZoneId = patch.lifeZoneId;
        if (patch.targetAudience !== undefined)
            record.targetAudience = patch.targetAudience;
        if (patch.prerequisiteLevel !== undefined)
            record.prerequisiteLevel = patch.prerequisiteLevel;
        if (patch.learningGoal !== undefined)
            record.learningGoal = patch.learningGoal;
        if (patch.unitBreakdown !== undefined)
            record.unitBreakdown = patch.unitBreakdown;
        if (patch.totalMinutes !== undefined)
            record.totalMinutes = patch.totalMinutes;
        if (patch.expectedOutcome !== undefined)
            record.expectedOutcome = patch.expectedOutcome;
        if (patch.sampleDescription !== undefined)
            record.sampleDescription = patch.sampleDescription;
        if (patch.capacity !== undefined)
            record.capacity = patch.capacity;
        if (patch.askingPrice !== undefined)
            record.askingPrice = patch.askingPrice;
        if (patch.feeBps !== undefined)
            record.feeBps = patch.feeBps;
        if (patch.policyVersionId !== undefined)
            record.policyVersionId = patch.policyVersionId;
     ...
     */
    cloneVersion(targetCourseId, tutorId, clonedFromCourseId) {
